        ...
        if (node.type === "ExportNamedDeclaration" && node.declaration && node.declaration.type === "TSInterfaceDeclaration") {
            ...
            if (node.declaration.extends && node.declaration.extends.length > 0) {
                try {
                    ...
                    console.warn(`  Warning: Could not parse extends clause for ${interfaceName}:`, error.message)
                }
            }
            ...
            node.declaration.body.body.forEach(member => {
                if (member.type === "TSPropertySignature") {
                    ...
                    if (isDateParameter(propName) && (propType === "number" || propType === "number | null")) {
        ...
        if (node.type === "ExportNamedDeclaration" && node.declaration && node.declaration.type === "FunctionDeclaration") {
            ...
            node.declaration.params.forEach(param => {
                if (param.type === "Identifier") {
                    ...
                    if (isDateParameter(paramName) && (paramType === "number" || paramType === "number | null")) {
...
function extractJSDocComment(node) {
    ...
    if (node.leadingComments) {
        const jsdocComment = node.leadingComments.find(comment => comment.type === "Block" && comment.value.trim().startsWith("*"))
        if (jsdocComment) {
    ...
    for (const [interfaceName, interfaceData] of Object.entries(interfaces)) {
        if (interfaceData.extends) {
            ...
            if (parentInterface) {
                ...
                const parentProperties = Array.isArray(parentInterface) ? parentInterface : parentInterface.properties
                ...
                console.warn(`  Warning: Parent interface ${interfaceData.extends} not found for ${interfaceName}`)
...
function generateManifestFromMultipleFiles(inputFiles, outputFile) {
    ...
    inputFiles.forEach(filePath => {
        ...
        console.log(`  Found ${Object.keys(api).length} functions and ${Object.keys(interfaces).length} interfaces`)
    })
...
`
    ...
    console.log(`📊 Total found: ${Object.keys(combinedAPI).length} functions and ${Object.keys(combinedInterfaces).length} interfaces`)
