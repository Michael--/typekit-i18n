...
function scanTranslationKeys() {
  ...
  if (keys.length > 0) {
    console.log(
      `  📝 Keys: ${keys.slice(0, 5).join(', ')}${keys.length > 5 ? `, ... (+${keys.length - 5} more)` : ''}`
    )
  }
  return keys
}
...
function generateSwiftMethod(funcName, funcInfo, interfaces) {
  ...
  const paramValidation = funcInfo.params
    .map((param) => {
      ...
      if (
        param.type === 'string' ||
        param.type === 'TranslateKeys' ||
        param.type === 'Iso639CodeType'
      ) {
  ...
  const description = funcInfo.description
    ? `    /// ${funcInfo.description.split('\n').join('\n    /// ')}\n`
    : ''
...
function main() {
  ...
  const outputPath = path.join(
    projectRoot,
    'native/Helio11/Helio11/generated/HelioJSAPI.generated.swift'
  )
  ...
  try {
    ...
    console.log(
      `📊 Generated ${Object.keys(manifest.functions).length} methods and ${Object.keys(manifest.interfaces).length} structs`
    )
